        ...
        WrappedCmp.getInitialProps = function (ctx) {
            return new _Promise(function (res) {
                ...
                Object.keys(output.actions).forEach(actionKey => {
                    ctx.actions[actionKey] = (...args) => dispatch(output.actions[actionKey](...args))
                });

                Object.keys(output.selectors).forEach(selectorKey => {
                    ctx.selectors[selectorKey] = () => output.selectors[selectorKey](ctx.store.getState())
                });
